...
static int MXC622X_ReadData(struct i2c_client *client, char data[MXC622X_AXES_NUM])
{
	...
	for(num=0;num<5;num++)
	{
		...
		{
			datax[num] = (signed char)(buf[0]);
			datay[num] = (signed char)(buf[1]);
			//printk("[+++++++ ACC MXC622X +++++++++++] %s: data[0] = %d, data[1] = %d\n", __FUNCTION__, buf[0], buf[1]);
		}
	}
	bubble(datax, 5);
	bubble(datay, 5);
		//printk("datax = %d %d %d %d %d\n", datax[0],datax[1],datax[2],datax[3],datax[4]);
		//printk("datay = %d %d %d %d %d\n", datay[0],datay[1],datay[2],datay[3],datay[4]);
	data[MXC622X_AXIS_X] = (signed char)(datax[0]);
	data[MXC622X_AXIS_Y] = (signed char)(datay[0]);
