...
#define TPD_KEYS_DIM            {{200,2100,109,100},{500,2100,109,100},{800,2100,109,100}}
...
//#ifdef ELAN_MTK6577
  ...
  if(!pReadData){
	...
	#endif
	printk("[MAMAMA] %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x\n", buf[0], buf[1], buf[2], buf[3], buf[4], buf[5], buf[6], buf[7],buf[8], buf[9], buf[10], buf[11], buf[12], buf[13], buf[14], buf[15],buf[16], buf[17]);
                ...
                printk("tp buf[btn_idx] = %x KEY_MENU=%x KEY_HOME=%x KEY_BACK=%x KEY_SEARCH =%x\n",buf[btn_idx], KEY_MENU, KEY_HOME, KEY_BACK, KEY_SEARCH);
					...
					switch (buf[btn_idx]) {
						...
						//printk("KEY back 1\n");
									input_report_key(idev, BTN_TOUCH, 1);
									input_report_abs(idev, ABS_MT_TOUCH_MAJOR, 8);
									input_report_abs(idev, ABS_MT_POSITION_X, 800);
									input_report_abs(idev, ABS_MT_POSITION_Y, 2100);
					...
					case ELAN_KEY_HOME:
						//printk("KEY home 1\n");
									input_report_key(idev, BTN_TOUCH, 1);
									input_report_abs(idev, ABS_MT_TOUCH_MAJOR, 8);
									input_report_abs(idev, ABS_MT_POSITION_X, 500);
									input_report_abs(idev, ABS_MT_POSITION_Y, 2100);
					...
					case ELAN_KEY_MENU:
						//printk("KEY menu 1\n");
									input_report_key(idev, BTN_TOUCH, 1);
									input_report_abs(idev, ABS_MT_TOUCH_MAJOR, 8);
									input_report_abs(idev, ABS_MT_POSITION_X, 200);
									input_report_abs(idev, ABS_MT_POSITION_Y, 2100);
				...
				{	
					if ((fbits & 0x01)) 
					{
                        ...
                        if(X_RESOLUTION > 0 && Y_RESOLUTION > 0)
                        {
                            //x = ( x * LCM_X_MAX )/X_RESOLUTION;
                            //y = ( y * LCM_Y_MAX )/Y_RESOLUTION;
							x = ( x * 1080 )/X_RESOLUTION;
                            y = ( y * 1920 )/Y_RESOLUTION;
                        }
                        else
                        {
                            //x = ( x * LCM_X_MAX )/ELAN_X_MAX;
