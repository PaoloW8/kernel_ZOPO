...
#define PK_ERR(a,...)
...
#if (defined(DCT_V6)||defined(DCT_V8)||defined(DCT_V9)||defined(DCT_E6)||defined(DCT_E9)||defined(DCT_V5)||defined(DCT_V7)||defined(DCT_E5)||defined(DCT_E7)||defined(DCT_V10)||defined(DCT_V11))
	...
	#define GPIO_TORCH_FLASH_MODE_M_GPIO  GPIO_MODE_00
#elif (defined(DCT_V2T)||defined(DCT_V2W))
	#define GPIO_TORCH_FLASH_EN GPIO75
	#define GPIO_TORCH_FLASH_EN_M_GPIO  GPIO_MODE_00
#else
...
extern void upmu_set_flash_dim_duty(kal_uint32 val);
 ...
 void kd2693_torch_write(int addr)
{
        int i;
         mt_set_gpio_out(GPIO_TORCH_FLASH_EN, GPIO_OUT_ONE);
        udelay(6);		
        for(i=0 ;i < 8;i++)
        {
            if(addr&0x80)
            {
                mt_set_gpio_out(GPIO_TORCH_FLASH_EN, GPIO_OUT_ZERO);
                udelay(2);
                mt_set_gpio_out(GPIO_TORCH_FLASH_EN, GPIO_OUT_ONE);
                udelay(4);
            }
...
static ssize_t flashlight_store_lighton(struct device_driver *ddri, const char *buf, size_t count)
{
        ...
        if(on > 0)
        {
#if (defined(DCT_V6)||defined(DCT_V8)||defined(DCT_V9)||defined(DCT_V10)||defined(DCT_E6)||defined(DCT_E9)||defined(DCT_V5)||defined(DCT_V7)||defined(DCT_E5)||defined(DCT_V936)||defined(DCT_E7)||defined(DCT_V11))
...
#elif (defined(DCT_V2T)||defined(DCT_V2W))
          kd2693_torch_write(0x02);//lvp
          kd2693_torch_write(0x22);//timout
          kd2693_torch_write(0xa1);
#else
			upmu_set_flash_en(1);
